...
class AbstractsTest(TestCase):
  def setUp(self):
    ...
    self.series1 = ConferenceSeries.objects.create(
        title="foobar",
        notes="A premier conference on foobar."
    )
    self.series2 = ConferenceSeries.objects.create(
        title="bazbat",
        notes="The international BazBat society.",
    )

    # Establish organizers
    self.organizer1 = Organizer.objects.create(
        name="Susan",
    )
  ...
  def test_series_list(self):
    ...
    self.assertContains(response, reverse(
      "series_detail", args=[self.series1.id]))
    self.assertContains(response, self.series2.title)
    self.assertContains(response, reverse(
        "series_detail", args=[self.series2.id]))
